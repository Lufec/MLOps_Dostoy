...
def process_args(args):
    ...
    logger.info("Dropping ClienteNo")
    df = df.drop(columns = ['ClienteNo'])
        
    logger.info("Correct NivelGerencial")
    df.NivelGerencial = df.NivelGerencial-1
    
    logger.info("Convert PrimeiraCompra to float")
    
    df['PrimeiraCompra'] = df['PrimeiraCompra'].str.replace(',','.')
